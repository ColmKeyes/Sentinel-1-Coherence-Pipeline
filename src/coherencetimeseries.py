...
class CoherenceTimeSeries:
    ...
    def multiple_plots(self,titles=None):
        ...
        # Get all variables that contain the strings "coherence" or "backscatter"
        coh_bsc_vars = [var for var in self.cube.variables if "coherence" in var or "backscatter" in var]
        # Group by the 'code' variable
        grouped = self.cube.groupby('code')
        # Loop through the grouped data and plot each variable in coh_bsc_vars
        for var in coh_bsc_vars:
            ...
            for i, (code, data) in enumerate(grouped):
                ...
                ax[i].legend()
                ax[i].set_ylim([0, 1])
                ax[i].tick_params(axis='both', which='both', length=5, width=2)  # Set visible ticks
                ax.autoscale(enable=True, axis='both', tight=True) # Set tight layout
                if titles:
